<template>
  <header class="flex justify-between items-center p-4 bg-[#2A2727] text-white">
    ...
    <!-- Navigation links -->
    <nav 
      :class="` fixed flex-col md:flex-row ${isOpen ? 'flex' : 'show'} md:flex space-y-4 md:space-y-0 space-x-0 md:space-x-6 text-xl justify-center items-center absolute md:static top-16 md:top-auto left-0 right-0 md:left-auto bg-[#2A2727] md:bg-transparent p-4 md:p-0`"
    >
      <NuxtLink to="/" class="">Home</NuxtLink>
      <NuxtLink to="/about" class="">About</NuxtLink>
      <NuxtLink to="/service" class="">Services</NuxtLink>
      <NuxtLink to="/Events" class="">Events</NuxtLink>
      <NuxtLink to="/Portfolio" class="">Portfolio</NuxtLink>
      <NuxtLink to="/Blog" class="">Blogs</NuxtLink> 
      <NuxtLink to="/Contact">
...
export default {
  ...
  data() {
    return {
      isOpen: false // State to manage menu visibility
    }
  },
  methods: {
    toggleMenu() {
